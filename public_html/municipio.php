...
$estado_sigla = isset($_GET['estado']) ? $_GET['estado'] : null;
$municipio_slug = isset($_GET['municipio']) ? $_GET['municipio'] : null;
$categorias = isset($_GET['categorias']) ? explode(',', $_GET['categorias']) : [];

// Verificar se os parâmetros necessários foram passados
if (!$estado_sigla || !$municipio_slug) {
...
$query_municipio = "
    ...
    WHERE e.sigla = ? AND m.slug = ?
";
$stmt_municipio = $conn->prepare($query_municipio);
$stmt_municipio->bind_param("ss", $estado_sigla, $municipio_slug);
...
// Buscar imagens da galeria do município (corrigido para tabela 'fotos')
...
$galeria = [];
while ($imagem = $resultado_galeria->fetch_assoc()) {
    $galeria[] = $imagem;
}

// Construir consulta para buscar parceiros
$query_parceiros = "
    SELECT p.*, c.nome as categoria_nome, c.slug as categoria_slug, t.nome as tipo_nome, t.slug as tipo_slug
    FROM parceiros p
    ...
    JOIN estados e ON m.estado_id = e.id
    WHERE e.sigla = ? AND m.slug = ? AND p.status = 1
";

// Adicionar filtro de tipos de parceiros, se houver
$params_tipos = [];
if (!empty($categorias)) {
    $placeholders = implode(',', array_fill(0, count($categorias), '?'));
    $query_parceiros .= " AND t.slug IN ($placeholders)";
    $params_tipos = $categorias;
}

$query_parceiros .= " ORDER BY p.destaque DESC, p.nome ASC";

// Preparar e executar consulta
$stmt_parceiros = $conn->prepare($query_parceiros);

// Vincular parâmetros
if (!empty($params_tipos)) {
    $tipos = 'ss' . str_repeat('s', count($params_tipos));
    $bind_params = [$tipos, $estado_sigla, $municipio_slug];
    foreach ($params_tipos as $cat) {
        $bind_params[] = $cat;
    }
    // Referências para bind_param
    $refs = array();
    foreach ($bind_params as $k => $v) {
        $refs[$k] = &$bind_params[$k];
    }
    call_user_func_array([$stmt_parceiros, 'bind_param'], $refs);
} else {
    $stmt_parceiros->bind_param("ss", $estado_sigla, $municipio_slug);
}

$stmt_parceiros->execute();
$resultado_parceiros = $stmt_parceiros->get_result();

// Obter todos os parceiros (agrupando por ID e juntando categorias)
$parceiros = [];
while ($parceiro = $resultado_parceiros->fetch_assoc()) {
    $id = $parceiro['id'];
    if (!isset($parceiros[$id])) {
        $parceiro['todas_categorias'] = [$parceiro['categoria_nome']];
        $parceiros[$id] = $parceiro;
    } else {
        $parceiros[$id]['todas_categorias'][] = $parceiro['categoria_nome'];
    }
}
// Reindexar para foreach
            ...
            <div class="container">
                ...
                <div class="municipio-filters">
                    ...
                    <div class="filter-categories">
                        <div class="category-option <?php echo empty($categorias) ? 'active' : ''; ?>" data-value="todos" onclick="window.location.href='municipio.php?estado=<?php echo $estado_sigla; ?>&municipio=<?php echo $municipio_slug; ?>'">Todos</div>
                        <div class="category-option <?php echo in_array('produtores', $categorias) ? 'active' : ''; ?>" data-value="produtores">Produtores</div>
                        <div class="category-option <?php echo in_array('criadores', $categorias) ? 'active' : ''; ?>" data-value="criadores">Criadores</div>
                        <div class="category-option <?php echo in_array('veterinarios', $categorias) ? 'active' : ''; ?>" data-value="veterinarios">Veterinários</div>
                        <div class="category-option <?php echo in_array('lojas-agropet', $categorias) ? 'active' : ''; ?>" data-value="lojas-agropet">Lojas Agropet</div>
                        <div class="category-option <?php echo in_array('cooperativas', $categorias) ? 'active' : ''; ?>" data-value="cooperativas">Cooperativas</div>
                    </div>
                </div>
                    ...
                    <div class="container">
                                ...
                                <?php foreach ($parceiros as $parceiro): ?>
                                    <div class="parceiro-card <?php echo $parceiro['destaque'] ? 'destaque' : ''; ?>">
                                        ...
                                        <div class="parceiro-content">
                                            ...
                                            <div class="parceiro-categoria"><?php echo implode(', ', $parceiro['todas_categorias']); ?><?php if(isset($parceiro['tipo_nome'])) echo ' - ' . $parceiro['tipo_nome']; ?></div>
                        ...
                        <?php else: ?>
                            <div class="sem-resultados">
                                ...
                                <a href="municipio.php?estado=<?php echo $estado_sigla; ?>&municipio=<?php echo $municipio_slug; ?>" class="btn-limpar-filtro">Limpar filtros</a>
                            </div>
                        <?php endif; ?>
                    </div>
                </section>
