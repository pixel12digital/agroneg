        ...
        <form method="post" action="parceiros.php?action=<?php echo $action; ?><?php echo ($action === 'edit') ? '&id=' . $id : ''; ?>" enctype="multipart/form-data">
            ...
            <div class="row mb-3 align-items-start">
                <div class="col-md-3 d-flex flex-column align-items-center">
                    ...
                    <div class="parceiro-aviso mt-2" style="font-size:0.98em; color:#555; background:#fffbe7; border-left:4px solid #ff9800; padding:8px 14px; border-radius:8px; box-shadow:0 1px 4px #0001; width:100%; max-width:220px;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    estadoSelect.addEventListener('change', function() {
        ...
        const estadoSigla = selectedOption.getAttribute('data-sigla');
    ...
    // Se estiver editando, carregar municípios do estado já selecionado
    <?php if ($action === 'edit' && isset($parceiro['estado_id'])): ?>
    const estadoIdEdit = '<?php echo $parceiro['estado_id']; ?>';
    const municipioIdEdit = '<?php echo $parceiro['municipio_id'] ?? ''; ?>';
    if (estadoIdEdit) {
        const selectedOption = estadoSelect.querySelector('option[value="' + estadoIdEdit + '"]');
        const estadoSigla = selectedOption ? selectedOption.getAttribute('data-sigla') : '';
        if (estadoSigla) {
            fetch('../api/municipios.php?estado=' + estadoSigla)
                .then(response => response.json())
                .then(data => {
                    ...
                    data.forEach(function(municipio) {
                        ...
                        option.value = municipio.id;
                        option.textContent = municipio.nome;
                        if (municipio.id == municipioIdEdit) option.selected = true;
                        municipioSelect.appendChild(option);
                    });
                    municipioSelect.disabled = false;
                });
